...
> Модель по умолчанию — полная `yandexgpt` (Pro): точнее разбирает даты,
> намерения и структуру. Чтобы сэкономить ценой качества, можно вернуть
> облегчённую: `LLM_MODEL=gpt://<YC_FOLDER_ID>/yandexgpt-lite/latest`.
