...
> Модель по умолчанию — **DeepSeek V4 Flash** через Yandex AI Studio: дешевле
> `yandexgpt-lite` и заметно сильнее в разборе (даты, намерения, структура),
> при этом остаётся в рублях и в РФ. В `LLM_MODEL` можно указать короткое имя
> модели из AI Studio (`deepseek-v4-flash`, `yandexgpt`, `yandexgpt-lite`,
> `qwen3-235b`) — полный URI код соберёт сам.
>
> Точный идентификатор модели показан в консоли AI Studio (у модели → «API»).
> Если бот отвечает «ИИ недоступен», впишите полный URI из консоли, напр.
> `LLM_MODEL=gpt://<YC_FOLDER_ID>/deepseek-v4-flash/latest`, и перезапустите бота.
